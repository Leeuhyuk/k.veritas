...
async function publishFiles(files, folder = 'misc') {
  ...
  for (const f of list) {
    ...
    const orig = (f.originalname || f.filename || 'file').replace(/[^\w.\-()\s\uac00-\ud7a3]+/g, '_');
    const safeDisp = orig.replace(/"/g, '');
    await bucket.upload(f.path, {
      ...
      metadata: {
        ...
        // 브라우저가 미리보기 대신 저장하도록
        contentDisposition: 'attachment; filename="' + safeDisp + '"; filename*=UTF-8\'\'' + encodeURIComponent(safeDisp),
      },
    });
    // 공개 읽기 URL (객체 ACL publicRead)
    ...
    try {
      ...
      console.warn('[media] makePublic 실패, signed URL 사용:', e.message);
      // makePublic 실패 시 signed URL (7일)
